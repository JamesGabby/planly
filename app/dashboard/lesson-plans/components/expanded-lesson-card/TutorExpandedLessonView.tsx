...
import { TutorPrintPreviewDialog } from "@/components/TutorPrintPreviewDialog";
import { generateTutorPrintContent } from "@/lib/utils/generateTutorPrintContent";
...
export function TutorExpandedLessonView({ lesson }: { lesson: LessonPlanTutor }) {
  ...
  return (
    <div id="lesson-print" className="ExpandedLessonView max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-6 sm:space-y-8">
      ...
      <TutorPrintPreviewDialog
        open={printDialogOpen}
        onOpenChange={setPrintDialogOpen}
        lesson={lesson}
        generatePrintContent={(sections, options) =>
          generateTutorPrintContent(lesson, sections, options)
        }
      />
    </div>
  );
