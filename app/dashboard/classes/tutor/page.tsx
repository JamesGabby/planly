...
export default function TutorClassesDashboard() {
  ...
  const fetchClasses = useCallback(async (userId: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          students:tutor_class_students(
            student:tutor_student_profiles(*)
          )
        `)
        .eq("user_id", userId)
    ...
    <div className="min-h-screen bg-background text-foreground p-6 transition-colors">
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Your Tutoring Classes</h1>
          ...
          <>
            ...
            >
              {paginated.map((cls) => (
                ...
                >
                  <Link
                    href={`/dashboard/classes/tutor/${cls.class_id}`}
