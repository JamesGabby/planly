// components/lesson-plans/TutorPrintPreviewDialog.tsx
"use client";
...
export interface TutorPrintOptions {
...
interface TutorPrintPreviewDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  lesson: LessonPlanTutor;
  generatePrintContent: (sections: string[], options: TutorPrintOptions) => string;
}

const defaultSections: PrintSection[] = [
  { id: "header", label: "Header & Student Info", icon: <LayoutTemplate size={16} />, enabled: true },
  { id: "objectives", label: "Objectives & Outcomes", icon: <Target size={16} />, enabled: true },
...
export function TutorPrintPreviewDialog({
  ...
  const [options, setOptions] = useState<TutorPrintOptions>({
    paperSize: "a4",
  ...
  const handleCopyToClipboard = useCallback(async () => {
    ...
    try {
      const plainText = generateTutorPlainText(lesson, enabledSections);
  ...
  // Generate preview content whenever sections or options change
  const previewContent = generatePrintContent(enabledSections, options);
                    ...
                    <div className="space-y-3">
                      ...
                      <RadioGroup
                        ...
                        onValueChange={(v) =>
                          setOptions((prev) => ({
                            ...prev,
                            paperSize: v as TutorPrintOptions["paperSize"],
                          }))
                        }
                    ...
                    <div className="space-y-3">
                      ...
                      <RadioGroup
                        ...
                        onValueChange={(v) =>
                          setOptions((prev) => ({
                            ...prev,
                            orientation: v as TutorPrintOptions["orientation"],
                          }))
                        }
                    ...
                    <div className="space-y-3">
                      ...
                      <RadioGroup
                        ...
                        onValueChange={(v) =>
                          setOptions((prev) => ({
                            ...prev,
                            fontSize: v as TutorPrintOptions["fontSize"],
                          }))
                        }
                        className="grid grid-cols-3 gap-2"
                      >
...
function generateTutorPlainText(
  lesson: LessonPlanTutor,
  sections: string[]
): string {
  ...
  if (sections.includes("header")) {
    ...
    text += `Student: ${lesson.tutor_student_profiles?.first_name} ${lesson.tutor_student_profiles?.last_name}\n`;
  ...
  if (sections.includes("structure") && lessonStructure.length > 0) {
    text += `LESSON STRUCTURE\n${"-".repeat(30)}\n`;
    lessonStructure.forEach((stage, i) => {
      text += `\nStage ${i + 1}: ${stage.stage}\n`;
      if (stage.duration) text += `Duration: ${stage.duration}\n`;
      if (stage.teaching) text += `Teaching: ${stage.teaching}\n`;
      if (stage.learning) text += `Learning: ${stage.learning}\n`;
      if (stage.assessing) text += `Assessing: ${stage.assessing}\n`;
      if (stage.adapting) text += `Adapting: ${stage.adapting}\n`;
    });
    text += "\n";
  }

  if (sections.includes("homework") && lesson.homework) {
    text += `HOMEWORK\n${"-".repeat(30)}\n${lesson.homework}\n\n`;
  }
